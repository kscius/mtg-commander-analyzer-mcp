...
async function main(): Promise<void> {
  ...
  const softIssues = rows.filter((row) => !row.readyToShip || row.categoriesBelow > 0);
  if (softIssues.length > 0) {
    console.warn(
      `\nbenchmarkDecks: ${softIssues.length} commander(s) below quality target (readyToShip / categories) — non-blocking`
    );
  }

  if (benchmarkHasHardFailures(rows)) {
    console.error('\nbenchmarkDecks: hard check failure(s):');
    for (const message of formatBenchmarkHardFailures(rows)) {
